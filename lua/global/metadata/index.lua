    ...
    for line in file:lines() do
        ...
        if line:match("#%s.+") then
            ...
            if string.sub(title, 1, 3) == "###" then
                roman = 0
                alphabet = alphabet + 1
                table.insert(insertbuf, pos,
                    string.format("%s%s", titleIndent .. string.rep(" ", 4),
                        to_alphabet_index(alphabet) .. string.format(" %s (%s)", line, tostring(i))))
                pos = pos + 1
            elseif string.sub(title, 1, 2) == "##" then
                alphabet = 0
                roman = roman + 1
                table.insert(insertbuf, pos,
                    string.format("%s%s", titleIndent .. string.rep(" ", 2),
                        to_roman(roman) .. string.format(" %s (%s)", line, tostring(i))))
                pos = pos + 1
            elseif string.sub(title, 1, 1) == "#" then
                roman, alphabet = 0, 0
                num = num + 1
                table.insert(insertbuf, pos,
                    string.format("%s%s", titleIndent, tostring(num) .. string.format(". %s (%s)", line, tostring(i))))
                pos = pos + 1
            end
        end
    end
...
local alphabeticalSearch = function()
    ...
    vim.api.nvim_create_autocmd("TextChangedI",
        { buffer = buf, callback = function() handleTextChangedEvent(win, buf) end })
end
            ...
            if type(check:match("")) ~= "string" then
                ...
                for j = 1, #files do
                    if dt.dataExists(files[j], "tags", data) and dt.dataExists(files[j], "type", mdtype) then
                        table.insert(insertbuf, (i + 1),
                            string.format("%s%s", indent .. " ", string.match(files[j], "([^/\\]+)%.md$")))
                    end
                end
            end
        end
        ...
        if type(check:match("")) ~= "string" then --expand filepaths only if theyre not already expanded
            ...
            for i = 1, #files do
                if dt.dataExists(files[i], "tags", data) and dt.dataExists(files[i], "type", mdtype) then
                    table.insert(insertbuf, (pos + 1),
                        string.format("%s%s", indent .. " ", string.match(files[i], "([^/\\]+)%.md$")))
                end
            end
...
M.buildBuf1 = function()
    ...
    vim.keymap.set("n", "l",
        function()
            local line = vim.api.nvim_get_current_line(); if not line:match("%[") and not line:match("%s") then M
                    .fileIndex({ link = line }) end
        end, { buffer = buf })
