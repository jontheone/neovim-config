...
M.ChangeNode = function()
    ...
    vim.keymap.set({"n", "i"}, "<CR>", function()
        ...
        if input then
            ...
            for i=1,#lines do
                local line = lines[i]:match("(^#%+file name:).+")
                if line then
                    lines[i] = line..vim.fn.expand("%:t")
                    break
                end
            end
            vim.api.nvim_buf_set_lines(vim.api.nvim_get_current_buf(), 1, #lines, false, lines)
            vim.cmd("w!")
            os.rename(path, input)
            vim.cmd("bd!")
